...
def random_add_border(lst_image):
    ...
    lst_image = [
        cv2.copyMakeBorder(image, r[0], r[1], r[2], r[3], borderType=cv2.BORDER_CONSTANT, value=(255, 255, 255)) for
        image in lst_image]
...
def extract_boundary(image):
    ...
    # Find contours, find rotated rectangle, obtain four vertices, and draw
...
def save_positive_to_debug(positive_pairs, components_a, components_b, mask_a, mask_b, output_folder="debug/matching"):
    ...
    for index_a, index_b in positive_pairs:
        ...
        output_name = os.path.join(output_folder,
                                   "a_%d_b_%d_lbl_%s.png" % (index_a, index_b, str(component_a['label'])))
